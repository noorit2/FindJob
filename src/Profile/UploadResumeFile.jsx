...
import AuthContext from "../Store/auth-context";
 const UploadResumeFile=forwardRef((probs,refr) => {
...
const listRef =ref(storage,`folder/${auth.currentUser.uid}/resume`);
...
const uploadFiles = (file) => {
  ...
  const sotrageRef = ref(storage,`folder/${auth.currentUser.uid}/resume/${file.name}`);
  ...
  uploadTask.on(
    ...
    () => { 
      ...
      setDoc(userRef, {resume:downloadURL
     }, { merge: true });
     
      let profile={
        ...ctx.profile,resume:downloadURL
      };
      ctx.setProfile(profile);
      });
      ...
      const deleteRef = ref(storage,`folder/${auth.currentUser.uid}/resume/${remove}`);
    ...
    // Uh-oh, an error occurred!
        ...
        <input type="file" className={probs.className} id={probs.id} name={probs.name} onChange={probs.onChange}  />

  );
});
export default React.memo(UploadResumeFile,()=>true);
